...
# Set working directory to where the zip codes file 'us_postal_codes2.csv'. This is my own working directory
...
# found a free dataset with zip information here - https://blog.splitwise.com/2013/09/18/the-2010-us-census-population-by-zip-code-totally-free/
zipcodes_df <- read.csv("us_postal_codes2.csv")
zipcodes_list <- zipcodes_df$Zip.Code.ZCTA
...
end <- 1000   # this is the ending county to stop at, after this many counties, parsing will stop
how_many_restarants <- 50000    # the number of restaurants to stop at, will take precedence over the ending county if the number of restauraunts has been reached first. Which will stop scraping/crawling
...
for (j in start:end){
        ...
        if (counter == 10){
                ...
                con <- file(description=paste("C:/Users/james/Documents/UC Riverside Masters - Data Science/Class - Information Retrieval/Project - Build Search Engine/YelpData_df", start, "-", j, ".csv", sep = ""), "w")
                write.csv(yelpData_df, con, row.names = FALSE)
                close(con)
                
                #write time_df to csv file
                con <- file(description=paste("C:/Users/james/Documents/UC Riverside Masters - Data Science/Class - Information Retrieval/Project - Build Search Engine/timeScrape_df", start, "-", j, ".csv", sep = ""), "w")
                write.csv(time_df, con, row.names = FALSE)
                close(con)
